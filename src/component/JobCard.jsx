...
const JobCard = ({ title, description, location, onShowMore, darkMode }) => {
  // Truncate description to first 100 characters to match NSS card
  ...
  return (
    <div className={`flex flex-col rounded-lg items-baseline justify-between shadow-lg p-4 sm:p-6 mb-4 sm:mb-6 transition-all hover:shadow-xl ${darkMode ? 'bg-gray-800 hover:bg-gray-750 text-white' : 'bg-white text-gray-800'}`}>
      <div>
        <h3 className="mb-2 text-lg sm:text-xl font-bold leading-tight">{title}</h3>
        <p className="mb-3 sm:mb-4 text-xs sm:text-sm text-justify leading-relaxed">{truncatedDescription}</p>
      </div>
