...
import { internshipPostings as jobPostings } from '../data/nssdata';
import NssModal from './NssModal';
import { FaThList, FaThLarge } from 'react-icons/fa';
import { useDebounce, usePerformanceMonitor } from '../hooks';

// eslint-disable-next-line react/prop-types
const NssBoard = memo(({ darkMode }) => {
  const { measureOperation } = usePerformanceMonitor('NssBoard');
  ...
  const filteredPostings = useMemo(() => {
    if (!debouncedSearchTerm) return jobPostings;
    
    return measureOperation('filterPostings', () => 
      jobPostings.filter((posting) =>
        posting.positionTitle?.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
        posting.department?.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
        posting.roleOverview?.toLowerCase().includes(debouncedSearchTerm.toLowerCase())
      )
    );
  }, [debouncedSearchTerm, measureOperation]);

  const handleShowMore = useCallback((posting) => {
    ...
    <div id="nsslist" className={`mt-4 max-w-7xl mx-auto py-6 sm:py-8 lg:py-12 px-2 sm:px-4 lg:px-6 transition-all duration-500 ${darkMode ? 'bg-gray-900 text-white' : ' text-gray-900'}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8 space-y-4 sm:space-y-0">
        ...
        <div className="flex items-center space-x-2 sm:space-x-4 justify-center sm:justify-end">
          <input
            type="text"
            placeholder="Search for NSS internship positions..."
            value={searchTerm}
            onChange={handleSearchChange}
            className={`flex-1 sm:w-64 lg:w-80 border-2 border-gray-300 p-2 sm:p-3 rounded-lg outline-none focus:border-red-600 text-sm sm:text-base transition-all duration-200 ${darkMode ? 'bg-gray-800 text-white placeholder-gray-400' : 'bg-white text-gray-900 placeholder-gray-500'}`}
            aria-label="Search NSS positions"
          />
